...
// LOOKBOOK_BASE（食譜原始資料）已經搬到 recipes-data.js 這個獨立檔案裡，
// 要記得在 index.html 裡讓 recipes-data.js 排在 script.js 之前載入。
...
function renderNav(){
  ...
  document.getElementById('sideNav').innerHTML = `
    ...
    <div style="margin-top:auto;padding:16px 20px 0 20px;border-top:1px solid var(--border-soft);">
      <div style="font-size:10.5px;color:var(--text-faint);line-height:1.6;margin-bottom:10px;">資料儲存在你目前這個瀏覽器裡，請定期刷新資料庫。</div>
      <button class="ghost danger small" style="width:100%;" onclick="resetAllData()">更新資料庫</button>
    </div>
  `;
...
function renderLookbook(main){
  ...
  const filterBtns = `<button class="filterbtn ${noFilters?'active':''}" onclick="clearLookbookFilters()">全部</button>` +
    FOOD_CATEGORIES.map(c=>`<button class="filterbtn ${lookbookCategoryFilters.has(c)?'active '+(CATEGORY_TAG_CLASS[c]||''):''}" onclick="toggleLookbookFilter('cat','${c}')">${c}</button>`).join('') +
    `<span style="width:1px;background:var(--border);margin:0 4px;"></span>` +
    COOKWARE.map(c=>`<button class="filterbtn ${lookbookBuildingFilters.has(c)?'active '+(COOKWARE_TAG_CLASS[c]||''):''}" onclick="toggleLookbookFilter('build','${c}')">${c}</button>`).join('');

  main.innerHTML = `
    <div class="page-head">
      <div class="eyebrow">Recipe Book</div>
      <h1>食譜查詢</h1>
      <p>可依名稱、食材、或效果關鍵字搜尋，分類跟廚具可以複選疊加（例如同時選「吃的」+「烹飪鍋」）</p>
    </div>
